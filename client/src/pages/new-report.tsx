...
import { Loader2, ArrowRight, Save, CheckCircle2, ShieldAlert, Sparkles, AlertCircle } from "lucide-react";
  ...
  const handleGenerate = async () => {
    if (!title.trim() || !description.trim()) {
      toast({
        ...
        description: "Please provide both a title and description of the incident.",
        variant: "destructive"
      });
      return;
    }
    ...
    <div className="max-w-4xl mx-auto py-8">
      ...
      <div className="flex items-center justify-center mb-12">
        <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg transition-colors ${step === "input" ? "bg-primary text-primary-foreground shadow-lg shadow-primary/30" : "bg-muted text-muted-foreground"}`}>1</div>
          <div className="w-16 h-1 bg-border rounded-full" />
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg transition-colors ${step === "processing" ? "bg-primary text-primary-foreground shadow-lg shadow-primary/30" : step === "review" ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}>2</div>
          <div className="w-16 h-1 bg-border rounded-full" />
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg transition-colors ${step === "review" ? "bg-primary text-primary-foreground shadow-lg shadow-primary/30" : "bg-muted text-muted-foreground"}`}>3</div>
        </div>
      </div>
        ...
        {step === "input" && (
          ...
          >
            <div className="text-center mb-8">
              <h1 className="text-4xl font-display font-bold mb-3">Describe the Incident</h1>
              <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
                Tell us what happened in your own words. Our AI will structure it into a formal report and suggest next steps.
              </p>
            </div>
            ...
            <Card className="p-8 shadow-xl border-border/50 bg-card/50 backdrop-blur-sm">
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-foreground/80 ml-1">Incident Title</label>
                  <Input 
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-foreground/80 ml-1">Detailed Description</label>
                  <Textarea 
        ...
        {step === "processing" && (
          ...
          >
            ...
            <p className="text-muted-foreground text-lg max-w-md">
              Our AI is reviewing your description, categorizing the threat, and generating a structured FIR report.
            </p>
          </motion.div>
        )}
        ...
        {step === "review" && analysis && (
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-display font-bold">Analysis Complete</h1>
                <p className="text-muted-foreground">Review the generated report and suggested actions.</p>
              </div>
              <Button onClick={handleSave} size="lg" className="btn-primary-glow bg-green-600 hover:bg-green-700">
              ...
              <div className="lg:col-span-2 space-y-6">
                <Card className="p-6 border-l-4 border-l-primary shadow-lg bg-card/80">
                  <div className="flex items-center gap-3 mb-6 pb-4 border-b border-border/50">
                    ...
                    <h3 className="text-xl font-bold">Structured Report (FIR Format)</h3>
                  </div>
              ...
              <div className="space-y-6">
                <Card className="p-6 bg-red-500/5 border-red-200/20 shadow-lg">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-bold text-lg text-red-500">Immediate Actions</h3>
                  </div>
                  <ul className="space-y-4">
                    {analysis.structuredReport.nextSteps.map((step: string, idx: number) => (
                ...
                <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                  <p className="text-sm text-blue-600 dark:text-blue-400 font-medium text-center">
                    This report has been automatically formatted for submission to cyber crime authorities.
                  </p>
                </div>
              </div>
            </div>
