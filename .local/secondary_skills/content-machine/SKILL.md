...
- Stories are **ephemeral and sequential** — viewers tap through in 2-3 seconds. There is no caption to read. The visual _is_ the entire message.
...
5. Output to `client/public/videos/`— served at`/videos/` on the project domain.

##### FFmpeg in the environment's NixOS
...
- Always resolve the domain with `echo $PROJECT_DOMAINS` before setting a canvas video shape URL. Never hardcode a domain — project domains may change between sessions.
