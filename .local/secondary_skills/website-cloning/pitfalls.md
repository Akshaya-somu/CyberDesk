...
- `width`/`max-width`(especially for buttons — don't let them fill 100%) -`height`(especially for images — maintain aspect ratio with`object-fit`)
...
**Solution**: Replace`index.css`entirely with plain CSS (no Tailwind/shadcn). For`package.json`, the only deps a clone needs are: `react`, `react-dom`, `vite`, `@vitejs/plugin-react`, and any required Vite plugins. Remove everything else.
