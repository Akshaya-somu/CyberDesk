...
**Important:** The `ffprobe -f lavfi`approach does not work reliably in some hosted environments. Instead, use`ffmpeg`with the`select`and`showinfo`filters, then parse`pts_time` values from stderr output. This is the tested, working approach.
...
async function extractFrames(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const timestamp = Math.min(
      segment.startTime + fraction * segment.duration,
      segment.endTime - 0.1,
    );

    const framePath = path.join(
      outputDir,
      `segment_${segment.index}_frame_${i + 1}.jpg`,
    );
...
try {
  ...
  if (err?.code === "content_policy_violation") {
    console.log(
      `Segment ${segment.index + 1}: Skipped (content safety filter)`,
    );
  } else {
...
async function exportTopClips(
  ...
  for (let i = 0; i < topScores.length; i++) {
    ...
    const outputPath = path.join(
      outputDir,
      `clip_${i + 1}_score_${score.overallScore.toFixed(1)}.mp4`,
    );
...
async function exportHighlightReel(
  ...
  const sortedByTime = [...topScores].sort(
    (a, b) =>
      segments[a.segmentIndex].startTime - segments[b.segmentIndex].startTime,
  );
  ...
  if (clipPaths.length === 1) {
    ...
    if (clipPaths.length === 2) {
      ...
      filterParts.push(`[a0][a1]acrossfade=d=${transitionDuration}[outa]`);
...
async function viralityAutoTrim(
  ...
  const scores = await scoreAllSegments(
    videoPath,
    segments,
    videoContext,
    tmpDir,
  ...
  switch (outputMode) {
    case "best": {
      ...
      console.log(
        `Best clip exported: ${outPath} (score: ${score.overallScore.toFixed(1)})`,
      );
    ...
    case "multiple": {
      ...
      console.log(
        `Exported ${clips.length} clips to ${path.join(outputDir, "top_clips")}/`,
      );
