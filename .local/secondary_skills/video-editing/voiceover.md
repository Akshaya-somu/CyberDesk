...
### 1. Connect ElevenLabs via platform integration

ElevenLabs is available as a platform connector. Set it up through the integration system:
...
async function addVoiceoverToVideo(
  ...
  if (typeof script === "string") {
    ...
    const segmentPaths = await generateSegments(
      script,
      tmpDir,
      apiKey,
      voiceId,
